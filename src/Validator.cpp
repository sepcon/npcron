...
static constexpr auto constFieldValidationRegexStr =
...
static constexpr auto constSubFieldValidationRegexStr =
    "(((([0-9])([0-9])*-([0-9])([0-9])*)|([0-9])([0-9])*|\\*)(\\/"
    "[1-9]+[0-9]*)*)";

static const char* gFieldNames[] = {"minute", "hour", "mday", "month", "wday"};
...
static std::string replace(std::string word, const std::string& with,
                           const std::string& by) {
  size_t pos = 0;
...
static void raiseBadSyntaxException(FieldType field,
                                    const std::string& expression) {
  BadSyntaxException::raise(std::string("Invalid field ") + gFieldNames[field] +
                            " with value: " + expression +
                            " - Note: Possible values must be in range:" +
                            getValidRangeInStringOf(field));
}
...
static std::string convertToNumberBasedExpression(
    std::string expression, const std::map<std::string, std::string>& theMap) {
...
Validator::Validator(const std::string& expression, FieldType field)
    : _field(field), _valid(false) {
  changeExpression(expression, field);
}

void Validator::changeExpression(const std::string& expression,
                                 FieldType field) {
  _field = field;
...
bool Validator::validate() {
  _valid =
      std::regex_match(_expression, std::regex(constFieldValidationRegexStr));
  if (_valid) {
    std::sregex_iterator itEnd;
    std::regex subExprRegex = std::regex(constSubFieldValidationRegexStr);
    for (auto it = std::sregex_iterator(_expression.begin(), _expression.end(),
                                        subExprRegex);
         it != itEnd; ++it) {
    ...
    BadSyntaxException::raise("Invalid systax!");
  }
  return _valid;
}

bool Validator::isValidFiedValue(int value) {
  return gValidFieldRanges[_field].first <= value &&
         gValidFieldRanges[_field].second >= value;
}
...
bool Validator::validateSubExpression(const std::string& subExpr) {
  ...
  if (contains(subExpr, '*')) {
    if (contains(subExpr, '/')) {
      ...
      if (step > 0) {
        _subExprInfo.emplace_back(subExpr, ExpType::StepWholeRange,
                                  gValidFieldRanges[_field].first,
                                  gValidFieldRanges[_field].second, step);
        _valid = true;
      } else {
        raiseBadSyntaxException(_field, subExpr);
      }
    } else {
      _subExprInfo.emplace_back(subExpr, ExpType::AnyValue,
                                gValidFieldRanges[_field].first,
                                gValidFieldRanges[_field].second);
    }
    ...
    if (contains(subExpr, '-')) {
      ...
      if (isValidRange(range.first, range.second, step)) {
        _subExprInfo.emplace_back(subExpr, ExpType::StepFromTo, range.first,
                                  range.second, step);
      } else {
        raiseBadSyntaxException(_field, subExpr);
      }
      ...
      if (step > 0 && isValidFiedValue(startValue)) {
        _subExprInfo.emplace_back(subExpr, ExpType::StepFrom, startValue,
                                  gValidFieldRanges[_field].second, step);
      } else {
        raiseBadSyntaxException(_field, subExpr);
      }
    ...
    if (idxOfMinus != std::string::npos) {
      ...
      if (isValidRange(range.first, range.second)) {
        _valid = true;
        _subExprInfo.emplace_back(subExpr, ExpType::Range, range.first,
                                  range.second);
      } else {
        raiseBadSyntaxException(_field, subExpr);
      }
    ...
    if (!isValidFiedValue(value)) {
      ...
      _valid = true;
      _subExprInfo.emplace_back(subExpr, ExpType::SingleValue,
                                gValidFieldRanges[_field].first,
                                gValidFieldRanges[_field].second, value);
    }
  }
